...
public abstract class CommonDtoAdapter<T, V> implements DtoAdapterInterface <T, V> {
	...
	public List<V> convertTo(List<T> tList) throws DtoConversionException{
		...
		if(!CollectionUtils.isEmpty(tList)) {
			Tracer.debug(LOG, getClass().getSimpleName(), method, "converting # " + tList.size() + " objects");
			vList = new ArrayList<V>();
			for (T t : tList) {
				vList.add(convertTo(t));
			}
			Tracer.debug(LOG, getClass().getSimpleName(), method, "vList # " + vList.size() + " size");
		}
		else {
			Tracer.debug(LOG, getClass().getSimpleName(), method, "collection is empty");
			vList = new ArrayList<V>(0);
		}
		return vList;
	}
